...
namespace Hephaestus.Repository.Elasticsearch.Provider
{
    internal class UpdateProvider<T> : ICommandProvider<T> where T : Entity
    {
        ...
        internal UpdateProvider(ElasticsearchConfig config)
        {
        ...
        public Task ExecuteAsync(EntityContextInfo<T> contextInfo, CancellationToken token)
        {
            return Polly.Policy
                ...
                .RetryAsync(2)
                .ExecuteAsync(() => UpdateAsync(contextInfo.Document, contextInfo.EntityType, token));
        }

        private async Task UpdateAsync<TDocument>(TDocument document, Type documentType, CancellationToken token) where TDocument : Entity
        {
