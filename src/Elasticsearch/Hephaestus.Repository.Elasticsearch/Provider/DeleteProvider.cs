...
namespace Hephaestus.Repository.Elasticsearch.Provider
{
    internal class DeleteProvider<T> : ICommandProvider<T> where T : Entity
    {
        ...
        internal DeleteProvider(ElasticsearchConfig config)
        {
        ...
        public Task ExecuteAsync(EntityContextInfo<T> contextInfo, CancellationToken token)
        {
            return Polly.Policy
                ...
                .RetryAsync(2)
                .ExecuteAsync(() => DeleteAsync(contextInfo.Document, contextInfo.EntityType, token));
        }

        private async Task DeleteAsync<TDocument>(TDocument document, Type documentType, CancellationToken token) where TDocument : Entity
        {
